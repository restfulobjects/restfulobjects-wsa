...
namespace RestfulObjects.Applib
{
    public class RORequest
    {
        ...
        {
            return new RORequest {_path = path};
        }

        public static RORequest To(Uri baseUri, Resource resource, params String[] pathElements)
        {
            return new RORequest {_baseUri = baseUri, _resource = resource, _pathElements = pathElements};
        }

        public static RORequest To(Uri baseUri, Resource resource, Dictionary<string, string> pathElementMap)
        {
            var resourceRequest = new RORequest
                {
                    _baseUri = baseUri,
                    _resource = resource,
        ...
        private Dictionary<string, string> _pathElementMap;
        public Uri _baseUri;
        ...
        private string PrefixBaseIfRequired(string path)
        {
            var baseUriStr = _baseUri.ToString();
            if (path.StartsWith(baseUriStr))
            {
                return path;
            }
...
// adapted from https://github.com/tavis-software/UriTemplates 
namespace Tavis
{
    using System;
    ...
    namespace UriTemplates
    {
        public class UriTemplate
        {
            ...
            private static string Encode(string p, bool allowReserved)
            {
                ...
                return System.Uri.EscapeUriString(p);
            }
